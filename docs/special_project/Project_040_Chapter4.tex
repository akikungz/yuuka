...
\hspace*{1.3em}
ภาพที่ \ref{fig4:ProjectStructure} ใช้ประกอบการอธิบายแนวคิดการจัดระเบียบโครงงานและไฟล์ประกอบของระบบ โดยในสภาพแวดล้อมจริงของการพัฒนาปัจจุบัน โครงสร้างถูกแยกเป็นหลายรีโพซิทอรีตามความรับผิดชอบของบริการ และแต่ละรีโพซิทอรีมีเอกสาร ไฟล์กำหนดค่า และ source code ของตนเองอย่างชัดเจน
...
\begin{mycustomitem}
    ...
    \item package.json และ bun.lock หรือไฟล์ lock ที่ใช้ควบคุม dependency ของแต่ละรีโพซิทอรี
    ...
    \item ไฟล์ deployment และ observability เช่น midori-development.yaml, momoi-development.yaml, redis.yaml, prometheus.yaml และ docker/docker-compose.yaml
...
\hspace*{1.3em}
ระบบแพลตฟอร์มคลาวด์ที่พัฒนาขึ้นใช้สถาปัตยกรรมแบบ Microservices ซึ่งประกอบด้วยบริการหลัก 3 ตัว คือ Midori, Momoi และ Yuzu โดยมีบริการสนับสนุนเพิ่มเติม เช่น Arisu สำหรับการยืนยันตัวตน PostgreSQL สำหรับข้อมูลธุรกรรม Redis สำหรับ cache และคิวงาน RustFS สำหรับพื้นที่จัดเก็บไฟล์ และชุด observability สำหรับติดตามสถานะของระบบ แต่ละบริการทำงานอิสระและสื่อสารกันผ่าน HTTP API และระบบคิวงานแบบ asynchronous เพื่อให้ระบบมีความยืดหยุ่นและรองรับการขยายตัวได้ง่าย
...
\begin{mycustomitem}
    ...
    \item \textbf{Tailwind CSS และ Radix UI} - เครื่องมือสำหรับออกแบบส่วนติดต่อผู้ใช้และองค์ประกอบ UI
    ...
    \item \textbf{Redis และ BullMQ} - ระบบ cache และคิวงานสำหรับการประมวลผลแบบ asynchronous
    \item \textbf{Docker และ Kubernetes Manifests} - สำหรับการ deploy และจัดการโครงสร้างพื้นฐานของระบบ
...
\begin{mycustomitem}
    ...
    \item \textbf{Instance และ Reverse Proxy API} - สำหรับจัดการเครื่องเสมือนและช่องทางเข้าถึงบริการ
    \item \textbf{Storage API} - สำหรับจัดการไฟล์ โฟลเดอร์ การแบ่งปัน และสิทธิ์การเข้าถึง
\end{mycustomitem}
...
\hspace*{1.3em}
แต่ละบริการหลักได้แก่ Midori, Momoi และ Yuzu มีการ export metrics ผ่าน OpenTelemetry และ Prometheus-compatible endpoints ตัวชี้วัดหลักที่ติดตาม ได้แก่ อัตราคำขอต่อวินาที latency percentile (p50, p95, p99) สถานะของ queue worker จำนวนงานในคิว และอัตราความสำเร็จของการจัดสรรเครื่องเสมือน ข้อมูลเหล่านี้จะถูกนำเสนอใน Grafana dashboard ที่ออกแบบแยกตามบริการ และรวมไว้ในหน้าภาพรวมของระบบทั้งหมด

\hspace*{1.3em}
Jaeger ถูกนำมาใช้เพื่อ trace การประมวลผลคำขอตั้งแต่ต้นทางจนปลายทาง (end-to-end tracing) ซึ่งช่วยในการระบุคอขวดและวิเคราะห์ latency ระหว่างบริการต่าง ๆ โดยเฉพาะในกระบวนการสร้างเครื่องเสมือนที่ผ่านทั้ง API ของ Momoi เข้าสู่ Redis Queue และ Yuzu ก่อนถึง Proxmox VE การมี trace ครบถ้วนช่วยให้ทีมพัฒนาสามารถแยกแยะว่าความล่าช้าเกิดขึ้นที่จุดใดของระบบได้อย่างแม่นยำ
...
\hspace*{1.3em}
จากข้อมูลสรุป ครั้งนี้คำขอทั้งหมดสำเร็จ (ค่า ok=\,45{,}146) สะท้อนว่าเส้นทางสาธารณะทำงานถูกต้องภายใต้เงื่อนไขการทดสอบดังกล่าวตามข้อมูลในตารางที่ \ref{tab:momoi-paths}
