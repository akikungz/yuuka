...
\begin{mycustomenum2}
    \item \textbf{Landing และ Login Page} สำหรับการเข้าถึงระบบและเข้าสู่กระบวนการยืนยันตัวตน
...
\begin{mycustomenum2}
    ...
    \item \textbf{Configurable Queue Concurrency} สำหรับปรับระดับการประมวลผลพร้อมกันของแต่ละคิวผ่านตัวแปรสภาพแวดล้อม
    \item \textbf{Redis-based Resource Locks} สำหรับป้องกันงานที่ชน instance เดียวกันหรือ `VMID` เดียวกันไม่ให้ทำงานซ้อนกัน
\end{mycustomenum2}

\hspace*{1.3em}
การแยก Yuzu ออกมาเป็น worker service ช่วยให้สามารถจัดการงานที่ใช้เวลานานและงานที่ต้อง retry ได้อย่างเหมาะสม โดยไม่ทำให้ API หลักต้องรอผลการทำงานจาก Proxmox VE โดยตรง อีกทั้งใน implementation ปัจจุบันยังสามารถเพิ่มหรือลด concurrency ของแต่ละคิวได้โดยไม่ต้องแก้โค้ด และใช้ distributed lock เพื่อจำกัดงานที่มีโอกาสชน resource เดียวกัน ช่วยลดความเสี่ยงต่อปัญหา storage lock timeout หรือการเปลี่ยนสถานะซ้ำซ้อนของเครื่องเสมือน

\subsection{รีโพซิทอรี Satsuki}
\hspace*{1.3em}
รีโพซิทอรี \textbf{Satsuki} เป็นบริการสนับสนุนด้านโครงสร้างพื้นฐานที่รับผิดชอบการสร้างไฟล์กำหนดค่า reverse proxy และไฟล์ `authorized\_keys` แบบอัตโนมัติจากข้อมูลในฐานข้อมูล พัฒนาด้วย Bun และจัดโครงสร้างภายในตามแนว clean architecture โดยมี `src/domain`, `src/application`, `src/infrastructure` และ `src/bootstrap` แยกตามบทบาทชัดเจน ในการติดตั้งใช้งานจริง Satsuki จะถูกนำไปติดตั้งบนเครื่อง Nginx ภายนอกซึ่งทำหน้าที่เป็น reverse proxy หน้าด่านของระบบ เพื่อให้สามารถเขียนไฟล์ configuration และ reload Nginx ได้โดยตรง
\begin{mycustomenum2}
    \item \textbf{Reverse Proxy Config Generator} สำหรับสร้าง Nginx stream/http configuration จากข้อมูล Instance และ reverse proxy mapping บนเครื่อง proxy ภายนอก
    \item \textbf{Authorized Keys Generator} สำหรับสร้างไฟล์กุญแจ SSH ที่ใช้กับ bastion หรือเครื่องกลางของระบบบนโฮสต์เดียวกัน
    \item \textbf{PostgreSQL LISTEN/NOTIFY Integration} สำหรับติดตามการเปลี่ยนแปลงของข้อมูลและ trigger การ regenerate configuration พร้อม reload Nginx โดยอัตโนมัติ
\end{mycustomenum2}
\hspace*{1.3em}
...
\hspace*{1.3em}
เมื่อพิจารณาการทำงานจริงของระบบในคลัสเตอร์ ผู้ใช้จะเริ่มจาก Browser ไปยัง Midori จากนั้น Midori จะเรียก Momoi สำหรับ API ด้านข้อมูล ตรรกะทางธุรกิจ และการยืนยันตัวตนผ่าน `/api/auth` ส่วนงานที่ใช้เวลานาน เช่น การสร้าง ลบ หรือเปลี่ยนสถานะเครื่องเสมือน Momoi จะส่งเข้าสู่ BullMQ บน Redis แล้วให้ Yuzu รับงานไปดำเนินการต่อกับ Proxmox VE ขณะเดียวกัน Satsuki ซึ่งทำงานอยู่บนเครื่อง Nginx ภายนอกจะติดตามการเปลี่ยนแปลงของข้อมูล reverse proxy และ SSH key จากฐานข้อมูลเพื่อ regenerate ไฟล์กำหนดค่าที่เกี่ยวข้องและ reload reverse proxy หน้าด่านโดยอัตโนมัติ รูปแบบนี้ทำให้ API หลักตอบสนองต่อผู้ใช้ได้รวดเร็ว แม้งานโครงสร้างพื้นฐานจะใช้เวลานานและต้อง retry หลายครั้ง

% \hspace*{1.3em}
% ภาพที่ \ref{fig4:SystemArchitecture} แสดงสถาปัตยกรรมระบบโดยรวม โดยมี \textbf{Midori} เป็นส่วนติดต่อผู้ใช้ที่พัฒนาด้วย Next.js และ React \textbf{Momoi} เป็นบริการ API หลักที่รับผิดชอบตรรกะทางธุรกิจ การเชื่อมต่อฐานข้อมูล การยืนยันตัวตน และการส่งงานเข้าคิว \textbf{Yuzu} เป็นบริการประมวลผลงานจัดการเครื่องเสมือนที่เชื่อมต่อกับ Proxmox VE \textbf{Satsuki} เป็นบริการสร้าง configuration สำหรับ reverse proxy และ SSH access ที่ถูกติดตั้งบนเครื่อง Nginx ภายนอก และ \textbf{Yuuka} เป็นแหล่งเก็บไฟล์กำหนดค่าการติดตั้งบริการประกอบทั้งหมดของแพลตฟอร์ม

% \begin{figure}[H]
% \centering
% \adjustbox{frame, width=1.0\textwidth}{
%     \includegraphics[width=1.0\textwidth]{Image/current_system_diagram.png}
% }
...
\subsection{ผลการทำงานของ Proxmox Cluster และการใช้หน่วยความจำร่วมด้วย KSM}
\label{sec:proxmox-cluster-perf}
\hspace*{1.3em}
ในส่วนของโครงสร้างพื้นฐานสำหรับเครื่องเสมือน ระบบถูกออกแบบให้ทำงานบน \textbf{Proxmox Cluster} จำนวนทั้งหมด 10 โหนด โดยแต่ละโหนดมีทรัพยากร \textbf{20 CPU} และ \textbf{32 GB RAM} ส่งผลให้ทั้งคลัสเตอร์มีทรัพยากรรวมเชิงกายภาพเท่ากับ \textbf{200 CPU} และ \textbf{320 GB RAM} ดังตารางที่ \ref{tab:proxmox-cluster-summary} ซึ่งเพียงพอสำหรับรองรับการใช้งานเครื่องเสมือนจำนวนมากในบริบทของห้องปฏิบัติการ รายวิชา และงานทดสอบที่มีรูปแบบการใช้งานคล้ายกันในหลายเครื่องพร้อมกัน

\begin{table}[htbp]
    \caption{\fontSixTeen{สรุปทรัพยากรของ Proxmox Cluster ที่ใช้ในระบบ}}\label{tab:proxmox-cluster-summary}
    \fontSixTeen
    \renewcommand{\arraystretch}{1.2}
    \begin{tabularx}{\textwidth}{|L|X|}
        \hline
        \textbf{รายการ} & \multicolumn{1}{|>{\centering\arraybackslash}X|}{\textbf{ค่า}} \\
        \hline
...
\hspace*{1.3em}
ผลลัพธ์ที่สำคัญของคลัสเตอร์นี้ไม่ได้มาจากจำนวนทรัพยากรเชิงกายภาพเพียงอย่างเดียว แต่ยังมาจากความสามารถของ Proxmox VE ในการใช้ \textbf{Kernel Samepage Merging (KSM)} เพื่อรวมหน้า memory ที่มีข้อมูลซ้ำกันระหว่าง VM หลายตัวเข้าด้วยกัน เมื่อเครื่องเสมือนจำนวนมากรันระบบปฏิบัติการตระกูลเดียวกัน หรือใช้ distro และ version ที่ใกล้เคียงกัน เช่น Ubuntu, Debian หรือ Linux รุ่นเดียวกัน จะมีส่วนของ page memory ที่ซ้ำกันอยู่เป็นจำนวนมาก KSM จะช่วยลดการเก็บข้อมูลซ้ำเหล่านี้ ทำให้หน่วยความจำที่ระบบต้องใช้จริงต่ำกว่าผลรวมของหน่วยความจำที่จัดสรรให้ VM แต่ละเครื่อง

\hspace*{1.3em}
คุณสมบัตินี้ทำให้คลัสเตอร์สามารถรองรับการใช้งานแบบ \textbf{memory overcommit} ได้อย่างมีประสิทธิภาพ กล่าวคือ แม้ผลรวมของ RAM ที่กำหนดให้กับ VM บนโหนดหนึ่ง ๆ จะมากกว่า \textbf{32 GB} ซึ่งเป็นหน่วยความจำจริงของเครื่อง แต่ระบบยังสามารถทำงานได้อย่างเสถียรตราบเท่าที่ working set ที่ถูกใช้งานจริงไม่ได้แตกต่างกันทั้งหมดทุกเครื่อง ในบริบทของระบบนี้ที่มักมี VM จำนวนมากสร้างจาก template เดียวกันและใช้งานแพ็กเกจพื้นฐานคล้ายกัน KSM จึงช่วยเพิ่มประสิทธิภาพการใช้ RAM ของโหนดได้อย่างชัดเจน ลดโอกาสที่ต้องใช้ swap เร็วเกินไป และช่วยให้สามารถจัดสรรเครื่องเสมือนได้หนาแน่นขึ้นโดยยังรักษาความต่อเนื่องของบริการ

\hspace*{1.3em}
ในเชิงผลลัพธ์ต่อการใช้งานจริง แนวทางดังกล่าวเหมาะกับสภาพแวดล้อมการศึกษาอย่างมาก เนื่องจากผู้ใช้งานจำนวนมากมักเริ่มต้นจาก image มาตรฐานชุดเดียวกัน ทำให้เกิดประโยชน์จาก KSM สูงกว่างานที่มี workload หลากหลายมาก ระบบจึงสามารถใช้ทรัพยากรหน่วยความจำได้คุ้มค่ากว่าเดิม รองรับการเปิดเครื่องพร้อมกันจำนวนมาก และทำให้การบริหารคลัสเตอร์ขนาด 10 โหนดมีความยืดหยุ่นมากขึ้นโดยไม่ต้องเพิ่มหน่วยความจำเชิงกายภาพในทันทีทุกครั้งที่มีการขยายจำนวน VM
...
\hspace*{1.3em}
อย่างไรก็ตาม หากพิจารณาในมุมของ \textit{time-to-usable-instance} หรือเวลาตั้งแต่เริ่ม provision จนผู้ใช้สามารถเข้าใช้งานเครื่องได้จริง ระยะเวลาที่รับรู้ได้จริงจะยาวกว่าค่าที่แสดงในตาราง เนื่องจากยังต้องรวมช่วง boot และขั้นตอน cloud-init ภายในเครื่อง โดยเฉพาะงาน \textit{packages update/upgrade} ที่อาจเพิ่มเวลาได้อีกราว 180--240 วินาที ทำให้แม้ Linked Clone จะเสร็จในระดับโครงสร้างพื้นฐานภายในไม่กี่วินาที แต่เวลาพร้อมใช้งานจริงของเครื่องอาจอยู่ในระดับประมาณ 3--4 นาที ขณะที่ Full Clone อาจสูงกว่านั้นอย่างมีนัยสำคัญ

\hspace*{1.3em}
นอกจากผลการเปรียบเทียบชนิดของการโคลนแล้ว ยังพบข้อสังเกตด้าน \textbf{throughput ของคิวงาน} ว่าประสิทธิภาพของ Yuzu ไม่ได้ขึ้นกับจำนวน worker pod บน Kubernetes เพียงอย่างเดียว แต่ขึ้นกับ \textbf{ผลคูณระหว่างจำนวน Pods และค่า concurrency ของแต่ละคิว} ด้วย ในสภาพแวดล้อมทดสอบนี้ได้กำหนดจำนวน worker ไว้ที่ \textbf{3 Pods} และกำหนดค่า concurrency ปริยายของคิว \textit{provision} เท่ากับ 5, \textit{deprovision} เท่ากับ 5 และ \textit{toggle status} เท่ากับ 10 ดังนั้นความสามารถเชิงทฤษฎีในการดึงงานขึ้นมาประมวลผลพร้อมกันจึงอยู่ที่ประมาณ \textbf{15 งาน} สำหรับ provision, \textbf{15 งาน} สำหรับ deprovision และ \textbf{30 งาน} สำหรับ toggle status หากงานเหล่านั้นไม่ชน resource lock เดียวกัน

\hspace*{1.3em}
ผลลัพธ์ที่สังเกตได้คือ เมื่อภาระงานยังไม่เกินความจุรวมของคิวที่เกี่ยวข้อง ระยะเวลาการเริ่มประมวลผลของแต่ละงานจะใกล้เคียงกับเวลาที่วัดจากการทดสอบ clone โดยตรง แต่เมื่อจำนวนงานสูงเกินกว่าความจุรวมของ worker สำหรับคิวนั้น หรือมีหลายงานอ้างถึง `instanceId` หรือ `VMID` เดียวกัน งานลำดับถัดไปจะยังไม่ล้มเหลวทันที แต่จะเข้าสู่สถานะรอคิว (\textit{queued/waiting}) หรือรอ lock ของทรัพยากรนั้นจนกว่าจะพร้อมประมวลผล ดังนั้นคอขวดในช่วงนี้ไม่ได้เกิดจากตรรกะภายใน Yuzu เพียงจุดเดียว แต่เกิดจากทั้งขีดจำกัดของจำนวน worker ที่พร้อมดึงงานจาก BullMQ และข้อจำกัดด้านความปลอดภัยของ resource locks ที่ใช้ป้องกันไม่ให้มีการจัดการ VM เดียวกันซ้อนกัน

\hspace*{1.3em}
อย่างไรก็ตาม การเพิ่ม concurrency เพียงอย่างเดียวไม่เพียงพอสำหรับงานที่แตะทรัพยากรเดียวกันใน Proxmox VE เช่น การสั่ง provision/deprovision/toggle บนเครื่องเสมือนเดียวกัน ด้วยเหตุนี้ Yuzu จึงเพิ่มกลไก \textbf{Redis-based distributed locks} เพื่อครอบ resource สำคัญสองระดับ ได้แก่ lock ตาม `instanceId` สำหรับกันไม่ให้ job หลายประเภทเข้ามาแก้ไข instance เดียวกันพร้อมกัน และ lock ตาม `VMID` สำหรับกันไม่ให้มีคำสั่งหลายชุดเข้าจัดการเครื่องเสมือนหมายเลขเดียวกันพร้อมกัน ไม่ว่าจะมาจากคิวชนิดใดก็ตาม แนวทางนี้ทำให้ระบบยังคงเปิดโอกาสให้หลายงานทำงานบน Proxmox node เดียวกันได้ หากเป็นคนละ `VMID` และไม่ชนกับเครื่องเดิม รวมถึงยังเอื้อให้การทำ Linked Clone จาก template เดียวกันสามารถเกิดขึ้นแบบขนานได้ เพราะ lock จะอ้างอิงที่ `VMID` ปลายทางของเครื่องใหม่ ไม่ใช่ template ต้นทาง

\begin{table}[H]
    \centering
    \caption{\fontSixTeen{พฤติกรรมของ Yuzu เมื่อจำนวนงานพร้อมกันสัมพันธ์กับจำนวน Pod และค่า concurrency}}
    \fontSixTeen
    \label{tab:yuzu-pod-queue-scaling}
    \begin{tabularx}{\textwidth}{|L|X|X|}
        \hline
        \textbf{ลักษณะของภาระงาน} & \multicolumn{1}{|>{\centering\arraybackslash}X|}{\textbf{สถานะของการประมวลผล}} & \multicolumn{1}{|>{\centering\arraybackslash}X|}{\textbf{ผลกระทบที่สังเกตได้}} \\
        \hline
        ...
        จำนวนงานชน `instanceId` หรือ `VMID` เดียวกัน & งานที่ชน resource เดียวกันจะถูกหน่วงไว้จนกว่างานก่อนหน้าจะปล่อย lock & throughput รวมของระบบยังสูงได้ แต่การจัดการ VM เดียวกันจะไม่ถูกทำซ้อนกันจนเกิด race condition \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    \centering
    \caption{\fontSixTeen{กลไกควบคุมการทำงานพร้อมกันของ Yuzu ฉบับปัจจุบัน}}
    \fontSixTeen
    \label{tab:yuzu-concurrency-and-locks}
    \begin{tabularx}{\textwidth}{|L|X|X|}
        \hline
        \textbf{กลไก} & \multicolumn{1}{|>{\centering\arraybackslash}X|}{\textbf{หน้าที่}} & \multicolumn{1}{|>{\centering\arraybackslash}X|}{\textbf{ผลลัพธ์ที่คาดหวัง}} \\
        \hline
        Queue Concurrency ต่อคิว & ปรับจำนวนงานที่หนึ่ง Pod ของ Yuzu รับพร้อมกันได้ในแต่ละคิวผ่าน environment variables & เพิ่ม throughput ต่อ Pod และลดเวลารอคิวเมื่อ workload กระจายได้ดี \\
        Lock ตาม `instanceId` & กันไม่ให้ job หลายชนิดเข้ามาเปลี่ยนสถานะหรือจัดการเครื่องเดียวกันพร้อมกัน & ลดความเสี่ยงจาก race condition และสถานะ VM ไม่สอดคล้องกัน \\
        Lock ตาม `VMID` & กันไม่ให้หลาย job เข้าจัดการเครื่องเสมือนหมายเลขเดียวกันพร้อมกัน แม้จะอยู่คนละคิว & ลดโอกาสสร้าง ลบ หรือเปลี่ยนสถานะ VM เดียวกันซ้อนกันจนเกิดความไม่สอดคล้อง \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{mycustomitem}
    ...
    \item ผลกระทบของจำนวน Pods ต่อคิวงาน: เมื่อเปิดใช้งาน Yuzu เพียง 3 Pods ระบบจะรองรับการประมวลผลพร้อมกันได้ดีที่สุดราว 3 งานในช่วงเวลาเดียวกัน หากมีงานมากกว่านี้ queue latency จะเพิ่มขึ้นทันทีแม้ตัว worker และ Proxmox VE ยังทำงานได้ปกติ
    \item ผลกระทบของ concurrency ภายใน Pod: การเปิดให้หนึ่ง instance ของ Yuzu ประมวลผลได้หลายงานพร้อมกันช่วยเพิ่มประสิทธิภาพการใช้ทรัพยากรของ Pod แต่จำเป็นต้องทำงานร่วมกับ resource locks เพื่อไม่ให้ throughput ที่เพิ่มขึ้นกลายเป็น contention กับ VM เดิมหรือทำให้สถานะของเครื่องเสมือนไม่สอดคล้องกัน
    \item ผลกระทบของ distributed locks: กลไก lock ตาม `instanceId` และ `VMID` ช่วยให้การเพิ่ม concurrency มีความปลอดภัยมากขึ้น โดยเฉพาะในงานที่มีโอกาสชน resource ซ้ำ เช่น provision ซ้อนกับ deprovision ของเครื่องเดิม หรือ toggle/deprovision ที่อ้างถึง VM ตัวเดียวกัน
\end{mycustomitem}
...
\begin{mycustomitem}
    ...
    \item หากคาดว่าจะมีการส่งงานพร้อมกันเกิน 3 งานเป็นประจำ ควรพิจารณาเพิ่มจำนวน Yuzu Pods หรือทำ autoscaling ตามความลึกของคิว เพื่อให้ queue waiting time ไม่กลายเป็นคอขวดหลักของระบบ
    \item ปรับค่า concurrency ของแต่ละคิวแยกจากกันตามลักษณะงานจริง เช่น จำกัด provision/deprovision มากกว่างาน toggle status ที่เบากว่า และติดตามผลกระทบหลังการปรับค่าทุกครั้ง
    \item ใช้ข้อมูลจาก lock timeout, queue depth และ active jobs ร่วมกันในการตัดสินใจว่าจะเพิ่ม Pod หรือเพียงปรับ concurrency ภายใน Pod เพื่อหลีกเลี่ยงการเพิ่มโหลดบน storage หรือ node โดยไม่จำเป็น
