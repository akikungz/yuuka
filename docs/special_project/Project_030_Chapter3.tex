...
\begin{mycustomitem}
    ...
    \item \textbf{Supporting Services and Deployment (Yuuka)} ชุดโครงสร้างพื้นฐานและไฟล์สำหรับการติดตั้งใช้งานจริง เช่น Arisu สำหรับการยืนยันตัวตน PostgreSQL, Redis, RustFS และระบบสังเกตการณ์การทำงานของแพลตฟอร์ม
\end{mycustomitem}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{Image/diagram/chapter3-dataflow-level2-request-approval.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Image/diagram/chapter3-dataflow-level2-request-provision.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{Image/diagram/chapter3-dataflow-level2-instance-queue.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\textwidth]{Image/diagram/chapter3-dataflow-level2-pve-execution.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{Image/diagram/database/db-request.png}
...
\begin{mycustomitem}
    ...
    \item \textbf{Documentation and Telemetry Endpoints} ใช้สำหรับสร้าง OpenAPI Documentation, Metrics และ Traces เพื่อสนับสนุนการทดสอบและการติดตามสถานะของระบบ
\end{mycustomitem}

\hspace*{1.3em} %ย่อหน้า
ตารางที่ \ref{tab3:api_groups} สรุปกลุ่ม API หลักของ Momoi พร้อมระดับการยืนยันตัวตนที่ต้องการ โดย Momoi เปิดเผย API เหล่านี้ผ่าน Elysia.js ซึ่งสร้าง OpenAPI Specification โดยอัตโนมัติ ทำให้ Midori สามารถนำข้อมูล Schema ไปสร้างชนิดข้อมูลฝั่ง Client ได้โดยตรงโดยไม่ต้องเขียนซ้ำ
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|}
        ...
        Auth & Login, Logout, Session, OAuth callback จาก Arisu \\
...
\hspace*{1.3em} %ย่อหน้า
การยืนยันตัวตนอาศัย Session ที่ออกโดย Arisu ซึ่งเป็น OAuth2-compatible authentication service โดย Midori จะ proxy คำขอ login ไปยัง Arisu และรับ session cookie กลับมา เมื่อเรียก API ของ Momoi ต่อไป Momoi จะตรวจสอบ session ผ่าน Arisu และดึงข้อมูล PlatformUser จากฐานข้อมูลเพื่อกำหนดบทบาท (ADMIN, INSTRUCTOR, STUDENT) ก่อนอนุมัติคำขอ การออกแบบในลักษณะนี้ทำให้ระบบ authentication อยู่ที่ Arisu เป็นศูนย์กลาง ขณะที่ Momoi รับผิดชอบเฉพาะ authorization ตามบทบาทที่กำหนดไว้ใน PlatformUser
...
\hspace*{1.3em} %ย่อหน้า
ข้อมูลที่ถูกส่งเข้าแต่ละงานจะเก็บเฉพาะตัวระบุที่จำเป็น เช่น instanceId, userId และสถานะที่ต้องการเปลี่ยนแปลง จากนั้น Yuzu จะอ่านข้อมูลฉบับเต็มจากฐานข้อมูลก่อนดำเนินการบน Proxmox VE วิธีการนี้ช่วยลดความซ้ำซ้อนของข้อมูลในคิวและทำให้การ retry งานมีความปลอดภัยมากขึ้น
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|}
        ...
        Toggle Status & instanceId, userId, targetStatus & สถานะ VM เปลี่ยนเป็น RUNNING STOPPED หรือ SUSPENDED ตามที่ระบุ \\
        \hline
    \end{tabularx}
\end{table}
...
\begin{mycustomitem}
    ...
    \item \textbf{Integration with API Proxy and OpenAPI Types} ช่วยให้ส่วน Frontend สามารถเชื่อมต่อกับ Momoi และ Arisu ได้อย่างมีชนิดข้อมูลที่ชัดเจน
\end{mycustomitem}
...
\hspace*{1.3em} %ย่อหน้า
\textbf{Observability Stack} ถูกนำมาใช้เพื่อวัดผลและติดตามสภาวะของระบบอย่างต่อเนื่อง โดยประกอบด้วย
\begin{mycustomitem}
