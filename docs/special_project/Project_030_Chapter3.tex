...
\begin{mycustomitem}
    ...
    \item \textbf{Worker / VM Management (Yuzu)} เซอร์วิสประมวลผลงานเบื้องหลังสำหรับการสร้าง ลบ และเปลี่ยนสถานะเครื่องเสมือน โดยสื่อสารกับ Proxmox VE แบบ Asynchronous รองรับการกำหนด concurrency ของแต่ละคิวผ่านตัวแปรสภาพแวดล้อม และมี distributed locks เพื่อป้องกันงานที่ชนทรัพยากรเดียวกัน
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{Image/current_system_diagram_90.png}
...
\hspace*{1.3em} %ย่อหน้า
\textbf{กระบวนการ 5.0 การประมวลผลงานจัดสรรเครื่องเสมือน} (ภาพที่ \ref{fig3:data_flow_level_1_vm}) แสดงบทบาทของ worker (Yuzu) ในการรับงานจากคิวและเรียก Proxmox VE จริง โดยผลลัพธ์ที่ได้จาก Proxmox VE (สำเร็จ/ล้มเหลว/สถานะระหว่างทาง) จะถูกนำกลับมาอัปเดตฐานข้อมูล เพื่อให้ Midori และ Momoi แสดงสถานะล่าสุดแก่ผู้ใช้ได้จากแหล่งข้อมูลเดียวกัน (single source of truth) ใน implementation ปัจจุบัน Yuzu ยังรองรับการประมวลผลหลายงานพร้อมกันภายในหนึ่ง instance และใช้กลไก distributed lock บน Redis เพื่อป้องกันไม่ให้ job ที่อ้างถึง `instanceId` เดียวกันหรือ `VMID` เดียวกันทำงานทับซ้อนกันจนเกิดปัญหา resource contention โดยไม่ปิดกั้นการทำงานพร้อมกันบนโหนดเดียวกันหากเป็นคนละเครื่อง
