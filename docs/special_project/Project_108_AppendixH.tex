...
\begin{mycustomenum2}
    ...
    \item \textbf{Yuuka} รับผิดชอบ manifest, Docker Compose สำหรับบริการประกอบ, ตัวอย่าง environment และการตั้งค่า observability ของระบบ
\end{mycustomenum2}
...
\begin{mycustomenum2}
    ...
    \item \textbf{งาน provision หรือเปลี่ยนสถานะ VM ไม่ทำงาน} ให้ตรวจสอบ Yuzu, Redis queue, การเชื่อมต่อไปยัง Proxmox VE และสถานะของ resource locks ว่ามีงานอื่นถือ lock ของ `instanceId` หรือ `VMID` เดียวกันอยู่หรือไม่
...
\begin{mycustomenum2}
    ...
    \item ตรวจสอบบริการประกอบจากคำสั่ง \textbf{docker compose -f docker/docker-compose.yaml ps} ในรีโพซิทอรี Yuuka
    \item ตรวจสอบ Redis และ PostgreSQL
    \item ตรวจสอบ log ของ Yuzu, ค่า concurrency ของแต่ละคิว และสถานะ queue/lock บน Redis
...
\begin{mycustomenum2}
    ...
    \begin{mycustomenum2}
        \item ตรวจสอบสถานะด้วยคำสั่ง: docker compose -f docker/docker-compose.yaml ps
        \item ดู Log ด้วยคำสั่ง: docker compose -f docker/docker-compose.yaml logs [service\_name]
        \item รีสตาร์ท Services ด้วยคำสั่ง: docker compose -f docker/docker-compose.yaml restart [service\_name]
    \end{mycustomenum2}
